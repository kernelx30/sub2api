...
router.beforeEach(async (to, _from, next) => {
  ...
  if ((to.meta.requiresPayment || to.meta.requiresRiskControl || to.meta.requiresMySubscriptions) && !appStore.publicSettingsLoaded) {
    try {
  ...
  if (
    to.meta.requiresMySubscriptions &&
    appStore.publicSettingsLoaded &&
    appStore.cachedPublicSettings?.my_subscriptions_enabled === false
  ) {
    next(authStore.isAdmin ? '/admin/dashboard' : '/dashboard')
    return
  }
