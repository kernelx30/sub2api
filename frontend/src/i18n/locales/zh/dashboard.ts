export default {
  ...
  keys: {
    ...
    groupLabel: '分组',
	openAIAvailabilityFallbackGroup: '备用 GPT 分组（可选）',
	openAIAvailabilityFallbackGroupHint: '主分组无可用账号或可重试失败耗尽时，按这里选定的第二分组继续请求。留空则只使用主分组。',
	noOpenAIAvailabilityFallbackGroup: '不使用备用分组',
	optionalInstructions: '破限提示词',
	optionalInstructionsHint: '注入该分组配置的破限提示词，降低模型的道德限制',
	optionalInstructionsUnavailable: '所选分组未配置破限提示词',
    selectGroup: '选择分组',
    statusLabel: '状态',
    selectStatus: '选择状态',
    saving: '保存中...',
