...
// CheckBillingEligibilityForGroupFailover re-checks the target group's billing
// and group RPM policy without incrementing the request-wide user RPM twice.
func (s *BillingCacheService) CheckBillingEligibilityForGroupFailover(ctx context.Context, user *User, apiKey *APIKey, group *Group, subscription *UserSubscription, platform string) error {
	return s.checkBillingEligibility(ctx, user, apiKey, group, subscription, platform, false)
}

func (s *BillingCacheService) checkBillingEligibility(ctx context.Context, user *User, apiKey *APIKey, group *Group, subscription *UserSubscription, platform string, includeUserRPM bool) error {
	// 简易模式：跳过所有计费检查
	...
	if err := s.checkRPMWithUserLimit(ctx, user, group, includeUserRPM); err != nil {
		return err
	}
...
func (s *BillingCacheService) checkRPMWithUserLimit(ctx context.Context, user *User, group *Group, includeUserLimit bool) error {
	...
	// ── 第二层：用户级全局硬上限（始终生效） ──
	if includeUserLimit && user.RPMLimit > 0 {
