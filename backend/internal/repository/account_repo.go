...
func (r *accountRepository) Update(ctx context.Context, account *service.Account) error {
	return r.updateAccount(ctx, account, nil, nil, nil, nil)
}
...
func (r *accountRepository) UpdateWithUpstreamBillingProbeEnabled(ctx context.Context, account *service.Account, enabled bool) error {
	return r.updateAccount(ctx, account, &enabled, nil, nil, nil)
}
...
func (r *accountRepository) UpdateWithProbeControls(ctx context.Context, account *service.Account, billingEnabled, poolAutoPriorityEnabled *bool) error {
	return r.updateAccount(ctx, account, billingEnabled, poolAutoPriorityEnabled, nil, nil)
}
...
func (r *accountRepository) UpdateWithAccountBillingSettings(
	...
	return r.updateAccount(ctx, account, probeEnabled, nil, rateSyncEnabled, rateMultiplier)
}
...
func (r *accountRepository) updateAccount(
	...
	explicitPoolAutoPriorityEnabled *bool,
	...
	updated, err := r.updateLockedAccount(ctx, client, account, explicitProbeEnabled, explicitPoolAutoPriorityEnabled, explicitRateSyncEnabled, explicitRateMultiplier)
	if err != nil {
		return translatePersistenceError(err, service.ErrAccountNotFound, nil)
	}
...
func (r *accountRepository) updateLockedAccount(
	...
	extra, err := lockAndMergeAccountProbeExtra(ctx, client, account, explicitProbeEnabled, explicitPoolAutoPriorityEnabled, explicitRateSyncEnabled)
	if err != nil {
		return nil, err
	}
	account.Extra = extra
...
func (r *accountRepository) updateUpstreamBillingProbeSnapshotInTx(
	...
	result, err := client.ExecContext(ctx, `
		...
		WHERE id = $2
			...
			AND COALESCE(extra -> 'upstream_billing_rate_sync_enabled', 'null'::jsonb) = $10::jsonb
			AND deleted_at IS NULL
	`, string(payload), account.ID, account.Platform, account.Type, string(credentials), proxyID, string(expectedSnapshotJSON), string(expectedEnabledJSON), string(expectedPoolAutoPriorityEnabledJSON), string(expectedRateSyncEnabledJSON), rateMultiplier)
	if err != nil {
		return err
	}
	affected, err := result.RowsAffected()
