...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	for {
		...
		selection, scheduleDecision, err := h.gatewayService.SelectAccountWithSchedulerForImagesAndSchedulingModel(
			requestCtx,
			apiKey.GroupID,
			sessionHash,
			routingModel,
			openAIChannelMappedRequestModel(routingModel, channelMapping),
			failedAccountIDs,
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &imageUpstreamErr) {
					...
					h.gatewayService.ReportOpenAIAccountScheduleResultFromForward(account.ID, account, openAIChannelMappedRequestModel(routingModel, channelMapping), "", false, result, !retryableServerError, nil)
				...
				if errors.As(err, &failoverErr) {
					h.gatewayService.ReportOpenAIAccountScheduleResultFromForward(account.ID, account, openAIChannelMappedRequestModel(routingModel, channelMapping), "", false, result, false, nil)
		...
		if result != nil {
			...
			h.gatewayService.ReportOpenAIAccountScheduleResultFromForward(account.ID, account, openAIChannelMappedRequestModel(routingModel, channelMapping), "", false, result, true, result.FirstTokenMs)
		} else {
			h.gatewayService.ReportOpenAIAccountScheduleResultFromForward(account.ID, account, openAIChannelMappedRequestModel(routingModel, channelMapping), "", false, nil, true, nil)
		}
